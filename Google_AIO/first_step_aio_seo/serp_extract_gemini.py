#!/usr/bin/env python3
"""
DataForSEO Gemini SERP extractor — pulls Gemini cited URLs and top-N organic non-cited URLs.

Makes two API calls:
  1. ai_optimization/gemini/llm_responses/live  →  Gemini answer + cited sources
...
Usage:
  python3 serp_extract_gemini.py "top shopify shipping software"
  python3 serp_extract_gemini.py "best delivery notification email tools"

Output:
  - Prints Gemini response text + cited URLs to console
  - Prints top-N organic results (deduplicated from Gemini citations) to console
...
def fetch_gemini(keyword: str) -> dict:
    """Call Gemini llm_responses with web_search enabled."""
...
def extract_gemini(items: list) -> tuple:
    """Return (response_text, cited_url_list) from Gemini llm_responses items.

    DataForSEO Gemini response structure:
      item["type"] == "message"
...
def extract_organic(items: list, exclude_urls: set) -> list:
    """Return top-N organic results, deduplicating any URLs already cited by Gemini."""
...
def run(keyword: str):
    ...
    # ── 1. Gemini answer + cited sources ─────────────────────────────────────
    print(f"  Calling Gemini llm_responses (model={GEMINI_MODEL}, web_search=True)…")
    gemini_data  = fetch_gemini(keyword)
    gemini_items = _check_task(gemini_data, "Gemini")
    if gemini_items is None:
        return

    gemini_types = sorted({i.get("type", "?") for i in gemini_items})
    print(f"  Gemini item types: {gemini_types}")

    gemini_text, gemini_cited = extract_gemini(gemini_items)
    cited_url_set = {e["url"] for e in gemini_cited}
    ...
    sep = "─" * 60
    print(f"\n{sep}")
    print(f"GEMINI ANSWER — {'FOUND' if gemini_text is not None else 'NOT FOUND'}")
    print(sep)
    if gemini_text:
        preview = gemini_text[:800] + ("…" if len(gemini_text) > 800 else "")
        print(f"\n{preview}\n")
        print(f"Cited URLs ({len(gemini_cited)}):")
        for i, e in enumerate(gemini_cited, 1):
            print(f"  {i:2}. [{e['title']}]  {e['url']}")
            if e.get("snippet"):
                print(f"       {e['snippet'][:120]}…")
    else:
        print("  No Gemini answer detected for this keyword.")

    # ── Print organic ─────────────────────────────────────────────────────────
    print(f"\n{sep}")
    print(f"TOP {TOP_N_ORGANIC} ORGANIC — NON-CITED (deduped from Gemini citations)")
    print(sep)
    for e in organic:
        print(f"\n  [{e['rank']:2}] {e['title']}")
        print(f"        {e['url']}")
    ...
    output = {
        ...
        "aio": {                          # key kept as "aio" for downstream compatibility
            "found":         gemini_text is not None,
            "response_text": gemini_text,
            "cited_urls":    gemini_cited,
        },
        "non_cited_organic":   organic,
        "_gemini_item_types":  gemini_types,
    }
    ...
    raw_gemini = os.path.join(raw_dir, f"serp_{safe_filename(keyword)}_raw_gemini.json")
    with open(raw_gemini, "w", encoding="utf-8") as f:
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 serp_extract_gemini.py <keyword>")
        sys.exit(1)
